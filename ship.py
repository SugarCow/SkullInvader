...
class Ship(Sprite):
    ship_images = [pg.transform.rotozoom(pg.image.load(f'images/Shipee.png'), 0, 1.0)]
    ship_explosion_images = [pg.transform.rotozoom(pg.image.load(f'images/Dead_shipee{n+1}.png'), 0, 1.0) for n in range(9)]
...
# #       game_over if the ships == 0
        ...
        self.sb.close_high_score_file()

    def update(self):
        if self.timer == self.timer_explosion and self.timer.is_expired():
            print('ship timer has expired it is now really dead ......')
            self.really_dead()
            self.sb.open_high_score_file()
            score = self.sb.high_score_file.readlines()
            a=0
            for line in score:
                for i in line:
                    if i.isdigit() == True:
            ...
            if int(a) < self.sb.score: 
                self.sb.high_score_file.truncate(0)
                self.sb.high_score_file.write(('%d' % self.sb.score))
                print('%d' % self.sb.score)
